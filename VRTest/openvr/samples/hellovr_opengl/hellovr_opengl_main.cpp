...
{
	SYSTEMTIME stUTC;
	FILETIME time = {}; // Query Performance Counter for Acquiring high-resolution time stamps.
	...
	//if (stUTC.wMilliseconds != milliseconds)
	{
		milliseconds = stUTC.wMilliseconds;

		// Print position and quaternion (rotation).
		dprintf("\n%lu, %s, x = %.5f, y = %.5f, z = %.5f, qw = %.5f, qx = %.5f, qy = %.5f, qz = %.5f",
			(stUTC.wMilliseconds) + (1000 * stUTC.wSecond) + (60 * 1000 * stUTC.wMinute), deviceName,
